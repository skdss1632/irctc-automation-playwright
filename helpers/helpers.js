...
async function clickWithRetry(bookingDateWidget, trainCoachSelector) {
  ...
  await verifyElementByText({text:label, widget:bookingDateWidget, timeout:60000});
...
function validatePassengerData(passengerData) {
  ...
  // Stations
  if (!passengerData.SOURCE_STATION || passengerData.SOURCE_STATION.trim() === "") {
    throw new Error("SOURCE_STATION is required and cannot be empty");
  }
  
  if (!passengerData.DESTINATION_STATION || passengerData.DESTINATION_STATION.trim() === "") {
  ...
  if (!validCoaches.includes(passengerData.TRAIN_COACH.toUpperCase())) {
    throw new Error(
      `Invalid TRAIN_COACH: "${passengerData.TRAIN_COACH}". Valid options: ${validCoaches.join(", ")}`
    );
  }
  ...
  if (passengerData.PREMIUM_TATKAL) {
    ...
    if (!acClasses.includes(passengerData.TRAIN_COACH.toUpperCase())) {
      throw new Error(
        `PREMIUM_TATKAL is only available for AC classes (${acClasses.join(", ")}). Selected: ${passengerData.TRAIN_COACH}`
      );
    }
  }
  ...
  if (!passengerData.PASSENGER_DETAILS || !Array.isArray(passengerData.PASSENGER_DETAILS)) {
  ...
  passengerData.PASSENGER_DETAILS.forEach((passenger, index) => {
    ...
    // Age rules
    if (passenger.AGE < 5) {
      console.warn(`Passenger ${passengerNum}: Children under 5 years travel free and don't need a separate ticket`);
    }
    
    if (passenger.AGE >= 60) {
      console.log(`Passenger ${passengerNum}: Senior citizen discount may apply (age: ${passenger.AGE})`);
    }
    ...
    if (!validGenders.includes(passenger.GENDER)) {
      throw new Error(
        `Passenger ${passengerNum}: Invalid GENDER "${passenger.GENDER}". Valid options: ${validGenders.join(", ")}`
      );
    }
    ...
    if (!validSeats.includes(passenger.SEAT)) {
      throw new Error(
        `Passenger ${passengerNum}: Invalid SEAT "${passenger.SEAT}". Valid options: ${validSeats.join(", ")}`
      );
    }
    ...
    if (passenger.FOOD) {
      if (!validFoodChoices.includes(passenger.FOOD)) {
        throw new Error(
          `Passenger ${passengerNum}: Invalid FOOD choice "${passenger.FOOD}". Valid options: ${validFoodChoices.join(", ")}`
        );
      }
    }
  });
  ...
  if (passengerData.BOARDING_STATION && passengerData.BOARDING_STATION.trim() !== "") {
    // Boarding station should not be the same as source
    if (passengerData.BOARDING_STATION === passengerData.SOURCE_STATION) {
      console.warn("BOARDING_STATION is same as SOURCE_STATION. This is redundant.");
    }
  }
